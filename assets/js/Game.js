...
export function startgame(gamelevel) {
    ...
    GameArea.GameScore = new SpiritsLib.TextComponent(0, 0, 2 * GameArea.canvas.width / 4, 50, 0, 0, "text", "#000000", "30px Arial");
    GameArea.GameScore.score = 0;
    // 進度條
    GameArea.Progress_bar_background = new SpiritsLib.RectComponent(GameArea.canvas.width - 50, 20, 25, 60, 0, 0, "#FFFFFF", 0.5);
    GameArea.Progress_bar = new SpiritsLib.RectComponent(0, 16, 30, 62, 0, 0, "#000000", 1);
...
export function loop() {
    ...
    if (GameArea.Rubbishs.length < 2 || Math.floor(Math.random() * 60) == 2) {
        ...
        RubbishImg.onload = () => {
            ...
            GameArea.Rubbishs.push(new RubbishsLib.Rubbish(RubbishImg.width, RubbishImg.height, x, -RubbishImg.height, 0, 3, RubbishImg, 1, 15));
...
export function endgame(winbool) {
    ...
    setTimeout(() => {
        ...
        if (winbool == false) {
            ...
            GameArea.GameEndRect = new SpiritsLib.RectComponent(GameArea.canvas.width, GameArea.canvas.height, 0, 0, 0, 0, "#FFFFFF", 0.5);
            GameArea.GameEndRect.update();
            GameArea.GameEnd = new SpiritsLib.TextComponent(0, 0, GameArea.canvas.width / 2 - 100, GameArea.canvas.height / 2, 0, 0, "text", "#000000", "40px Arial");
